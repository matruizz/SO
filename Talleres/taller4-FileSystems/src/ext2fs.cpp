...
struct Ext2FSInode *Ext2FS::load_inode(unsigned int inode_number)
{
    ...
    //Dado el numero de inodo consigo el numero de blockGroup en el que esta ese inodo
    int blockGroupNumber = blockgroup_for_inode(inode_number);

    //Dado el numero de blockGroup consigo el descriptor del blockGroup en el que esta el inodo
    struct Ext2FSBlockGroupDescriptor *blockGroupDescriptor = block_group(blockGroupNumber);

    //Dado el descriptor de blockGroup consigo el puntero a la tabla de inodos
    ...
    {
        inode_buffer[i] = buffer[inodeIndexInBlock + i];
    }
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode *inode, unsigned int block_number)
{
    ...
    // TODO: Ejercicio 1
    int numberOfDirectBlocks = 12;
    int indexOfSimpleIndirectBlock = numberOfDirectBlocks;
    int indexOfDoubleIndirectBlock = indexOfSimpleIndirectBlock + 1;
    int numberOfEntriesInBlock = block_size / 4;
    int numberOfSimpleIndirectBlocks = numberOfEntriesInBlock;

    if (block_number < numberOfDirectBlocks)    //block_number corresponde a un bloque directo
    {

        return ((unsigned int *)inode->block)[block_number];
    }
    else if (numberOfDirectBlocks <= block_number && block_number < numberOfEntriesInBlock + numberOfDirectBlocks)  //block_number corresponde a un bloque indirecto simple
    {

        unsigned char *simpleIndirectBlock = (unsigned char *)malloc(block_size);

        read_block(inode->block[indexOfSimpleIndirectBlock], simpleIndirectBlock);

        int indexOfDataBlock = block_number - numberOfDirectBlocks;

        return ((unsigned int *)simpleIndirectBlock)[indexOfDataBlock];
    }
    else	 //block_number corresponde a un bloque indirecto doble
    {

        unsigned char *doubleIndirectBlock = (unsigned char *)malloc(block_size);

        read_block(inode->block[indexOfDoubleIndirectBlock], doubleIndirectBlock);

        // dividido 256 modulo 256
        int numberOfSimpleIndirectBlockInDoubleIndirectBlock = block_number - numberOfDirectBlocks - numberOfSimpleIndirectBlocks;
        indexOfSimpleIndirectBlock = numberOfSimpleIndirectBlockInDoubleIndirectBlock / numberOfEntriesInBlock;

        unsigned char *simpleIndirectBlock = (unsigned char *)malloc(block_size);
        read_block(((unsigned int *)doubleIndirectBlock)[indexOfSimpleIndirectBlock], simpleIndirectBlock);

        int indexOfDataBlock = numberOfSimpleIndirectBlockInDoubleIndirectBlock % numberOfEntriesInBlock;

        // si no funca poner cast
        return ((unsigned int *)simpleIndirectBlock)[indexOfDataBlock];
    }
}
