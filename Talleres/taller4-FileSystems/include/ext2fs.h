...
struct __attribute__((__packed__)) Ext2FSInode {
	unsigned short mode;	//File type and access rights
	unsigned short uid;		//Owner identifier
	unsigned int size;		//File length in bytes
	unsigned int atime;		//Time of last file access
	unsigned int ctime;		//Time that inode last changed 
	unsigned int mtime;		//Time that file contents last changed
	unsigned int dtime;		//Time of file deletion
	unsigned short gid;
	unsigned short links_count;
	unsigned int blocks;	//Number of data blocks of the file
	unsigned int flags;
